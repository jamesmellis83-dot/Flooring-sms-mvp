...
router.get('/dashboard', auth.requireAuth, (req, res) => {
  ...
  res.send(layout({ title: 'Dashboard — BidBuddy', user: req.user, company: co, active: 'dashboard', body: `
    ...
    <div class="card">
      ${recent.length ? `<table>
        <tr><th>Customer</th><th>Address</th><th>Estimator</th><th>Squares</th><th>Price</th><th>Date</th></tr>
        ${recent.map(e => `<tr>
          <td>${esc(e.customer_name || 'Untitled job')}</td>
          <td>${esc(e.job_address || '—')}</td>
...
router.get('/estimates', auth.requireAuth, (req, res) => {
  ...
  res.send(layout({ title: 'Estimates — BidBuddy', user: req.user, company: req.company, active: 'estimates', body: `
    ...
    <div class="card">
      ${rows.length ? `<table>
        <tr><th>Customer</th><th>Address</th><th>Estimator</th><th>Squares</th><th>Shingle</th><th>Cost</th><th>Price</th><th>Date</th></tr>
