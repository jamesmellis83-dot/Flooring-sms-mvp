...
// time it doesn't exist - it does NOT retroactively add new columns to a
// table that was already created on a previous deploy (e.g. production on
// Render). Any new pricing_profiles column added after go-live must be
// migrated in here instead, via ALTER TABLE wrapped in try/catch (SQLite has
// no clean "ADD COLUMN IF NOT EXISTS", so attempting-and-ignoring-the-error
// is the standard safe/idempotent pattern - it will fail harmlessly on every
// run after the first time the column already exists).
function migrateColumn(table, column, definition) {
  try {
    db.exec(`ALTER TABLE ${table} ADD COLUMN ${column} ${definition}`);
  } catch (_) {
    // Column already exists - nothing to do. Any other failure here is not
    // fatal to app startup; the column will simply behave as NULL/unset,
    // which downstream code already treats safely via the num()/fallback
    // helpers in lib/roofingPricing.js.
  }
}

// Valley linear-foot rate and skylight flashing fee, added so these are
// priced the same way ridge/eave and chimney/penetration already are,
// instead of being captured from the estimator's text and then silently
// dropped with zero effect on the price for real paying companies.
migrateColumn('pricing_profiles', 'valley_per_lf', 'REAL DEFAULT 12');
migrateColumn('pricing_profiles', 'skylight_flash_fee', 'REAL DEFAULT 275');

// *** FIX: customer_name / job_address must exist on every deploy, even ones
// that were created before these columns were added to schema.sql. Without
// this migration, an older production database (e.g. Render's persistent
// disk) would silently reject or drop these values on every INSERT/UPDATE -
// which looks EXACTLY like "customer name isn't saving", but for a totally
// different reason than the app-layer bug this file also fixes. Running
// this identically for every table/company means the fix applies uniformly
// across all tenants, not just newly created ones.
migrateColumn('estimates', 'customer_name', 'TEXT');
migrateColumn('estimates', 'job_address', 'TEXT');
...
// *** FIX: this function did not exist before. smsHandler.js's saveEstimate()
// call was previously INSERT-only, so a customer name/address arriving in a
// LATER message than the measurements (or any follow-up change) had no way
// to update the row already on the dashboard - it either silently did
// nothing, or (if it had been wired to call saveEstimate again) would have
// created a duplicate row per message. This gives every tenant exactly ONE
// row per job that gets refreshed in place as more details arrive.
//
// Returns true if a row was actually updated (used by callers to detect a
// stale/deleted dbEstimateId and self-heal by falling back to an insert),
// false if no matching row existed for this id + company_id.
function updateEstimate(id, companyId, { customer_name, job_address, squares, estimate_amount, estimate_json, source }) {
  const info = db.prepare(`
    UPDATE estimates
    SET customer_name = ?,
        job_address = ?,
...
function getEstimateById(id, companyId) {
  return db.prepare(`SELECT * FROM estimates WHERE id = ? AND company_id = ?`).get(id, companyId) || null;
}
...
module.exports = {
  ...
  saveEstimate, updateEstimate, getEstimateById, listEstimates, estimateStats, statsByUser,
