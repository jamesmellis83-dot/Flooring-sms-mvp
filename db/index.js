...
// UPSERT: if an `id` is passed in (an existing estimate row for this SMS
// conversation/job), UPDATE that exact row in place. Otherwise INSERT a new
// row. This is what lets follow-up messages - like a customer name/address
// that arrives AFTER the estimate was already computed and saved - update
// the same dashboard row instead of silently being dropped or, worse,
// creating a duplicate row for the same job every time a detail changes.
//
// Returns the row's id either way, so the caller (lib/smsHandler.js) can
// remember it on the in-memory conversation object and pass it back in on
// every subsequent save for that same job.
function saveEstimate({ id, company_id, created_by_user_id, customer_name, job_address, squares, estimate_amount, estimate_json, source }) {
  if (id) {
    const result = db.prepare(`
      UPDATE estimates
      SET customer_name = ?, job_address = ?, squares = ?, estimate_amount = ?, estimate_json = ?, source = ?
      WHERE id = ? AND company_id = ?
    `).run(customer_name || null, job_address || null, squares || null, estimate_amount,
           JSON.stringify(estimate_json || {}), source || 'sms', id, company_id);

    // If the row actually existed and belonged to this company, we're done.
    if (result.changes > 0) return id;
    // Otherwise (stale id, row was deleted, or company_id mismatch), fall
    // through and insert a fresh row instead of silently losing the estimate.
  }
