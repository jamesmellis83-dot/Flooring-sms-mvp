...
function formatRoofingReply(estimate) {
  ...
  lines.push("BidBuddy Roofing Estimate");
  lines.push(`Estimated total: ${money(estimate.total)}`);
  lines.push("");
  lines.push(`Roof size: ${estimate.squares} squares`);
  lines.push(`Material: ${estimate.materialType}`);
  lines.push(`Tear-off: ${estimate.tearOffLayers} layer(s)`);
  lines.push(`Pitch: ${estimate.pitch}`);
  lines.push(`Story count: ${estimate.storyCount}`);
  lines.push("");
  ...
  if (estimate.minimumApplied) {
    lines.push("- Minimum job price applied");
  }

  lines.push("");
  lines.push("Review measurements, material selection, access, decking, and local job conditions before sending a final customer quote.");
