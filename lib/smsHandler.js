...
const NAME_TRAILING_STOPWORDS = new Set(["at", "on", "near", "address", "is", "for", "and", "located", "near"]);
...
async function processIncomingSms({ from, to, text, companyId }) {
  ...
  if (currentEstimate) {
    ...
    // existing row in place instead of inserting a new one. If saveEstimate
    // returns the row's id, we remember it for all future saves in this job.
