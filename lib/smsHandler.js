 ...
 * - Existing pricing modules if available.
...
// -----------------------------------------------------------------------------
// Optional pricing module loading
// -----------------------------------------------------------------------------
...
function callExistingPricing(job) {
  if (!pricingModule) return null;
  ...
  for (const fnName of functionCandidates) {
    const fn = pricingModule[fnName] || (fnName === "default" && typeof pricingModule === "function" ? pricingModule : null);
...
function fallbackEstimate(job) {
  ...
  // if your project pricing engine is not available or has a different interface.
...
function calculateEstimate(job) {
  const existing = callExistingPricing(job);
  if (existing) {
    const low = existing.low || existing.min || existing.estimateLow || existing.totalLow || existing.baseLow;
    const high = existing.high || existing.max || existing.estimateHigh || existing.totalHigh || existing.baseHigh;
    if (low && high) {
      return {
        low: Math.round(Number(low)),
        high: Math.round(Number(high)),
        deckingLow: Number(existing.deckingLow || 0),
        deckingHigh: Number(existing.deckingHigh || 0),
        totalLow: Math.round(Number(existing.totalLow || low)),
        totalHigh: Math.round(Number(existing.totalHigh || high)),
        detailAdders: existing.detailAdders || [],
...
function estimateReply(job, conversation) {
  ...
  const estimate = calculateEstimate(job);
...
function homeownerHelpReply(job) {
  const range = calculateEstimate(job);
...
async function handleSms(req, res) {
  try {
    ...
    if (res) return res.status(200).json({ ok: true, reply: result.reply, intent: result.intent });
...
module.exports = {
  handleSms,
  handleInboundSms: async ({ from, text }) => {
    const result = await processIncomingSms({
      from,
      text
    });
