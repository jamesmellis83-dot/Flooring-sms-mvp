 ...
 *
 * NOTE ON gpt-5-mini: this model only supports the DEFAULT temperature
 * (1) and rejects `max_tokens` in favor of `max_completion_tokens`.
 * Sending either `temperature: <anything but 1>` or `max_tokens` will
 * cause OpenAI to return a 400 invalid_request_error, which this module
 * treats as a normal failure and falls back to the template reply.
 */

const DEFAULT_MODEL = process.env.OPENAI_MODEL || "gpt-5-mini";
const DEFAULT_TIMEOUT_MS = Number(process.env.AI_REPLY_TIMEOUT_MS || 8000);
const MAX_REPLY_TOKENS = Number(process.env.AI_REPLY_MAX_TOKENS || 300);
...
async function generateAiReply({
  ...
  try {
    ...
    if (!reply) {
      console.error("[BidBuddy AI] OpenAI response contained no content. Using template fallback.");
      return null;
    }
