 ...
 * grounding data assembled by lib/promptBuilder.js. This module owns:
...
const DEFAULT_TIMEOUT_MS = Number(process.env.AI_REPLY_TIMEOUT_MS || 8000);
const MAX_REPLY_TOKENS = Number(process.env.AI_REPLY_MAX_TOKENS || 260);
...
function sanitizeForSms(text) {
  ...
  const replacements = [
    ...
    [/(^|\n)\s*[*#]\s*/g, "$1- "],        // markdown bullets/headers -> hyphen
  ];
  ...
  out = out.replace(/[^\x00-\x7F\u00A3\u00A5\u00E8\u00E9\u00F9\u00EC\u00F2\u00C7\u00D8\u00F8\u00C5\u00E5\u0394\u03A6\u0393\u039B\u03A9\u03A0\u03A8\u03A3\u0398\u039E\u00C6\u00E6\u00DF\u00C9\u00A4\u00A1\u00C4\u00D6\u00D1\u00DC\u00A7\u00BF\u00E4\u00F6\u00F1\u00FC\u00E0]/g, "");

  return out.replace(/[ \t]+\n/g, "\n").trim();
}

async function generateAiReply({ companyName, job, missingFields, currentEstimate, previousEstimate, history, userText }) {
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages,
        temperature: 0.4,
        max_completetion_tokens: MAX_REPLY_TOKENS
      }),
    ...
    if (!response.ok) {
      ...
      console.error("[BidBuddy AI] OpenAI call failed", response.status, errBody);
      return null;
    }
    ...
    if (!content || !String(content).trim()) {
      console.error("[BidBuddy AI] OpenAI returned empty content.");
      return null;
    }

    return sanitizeForSms(content);
  } catch (error) {
    clearTimeout(timeout);
