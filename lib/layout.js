// lib/layout.js — one tiny server-rendered HTML shell for the portal.
const esc = (s) => String(s ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));

function layout({ title = 'BidBuddy', contractor = null, active = '', body = '' }) {
  const nav = contractor ? `
    <nav class="nav">
      ...
      <a href="/pricing-profile" class="${active==='pricing'?'on':''}">Pricing</a>
...
.link{background:none;border:0;color:var(--mut);font:inherit;font-size:15px;margin-left:18px;cursor:pointer}
...
.ok{background:#f0fdf4;border:1px solid #bbf7d0;color:#166534;padding:11px 14px;border-radius:7px;margin-bottom:16px;font-size:14px}
...
<header><div class="wrap bar">
  <a class="logo" href="${contractor ? '/dashboard' : '/'}">Bid<span>Buddy</span></a>
  ${nav}
</div></header>
