...
function jobBullets(job) {
  ...
  if (job.pipeBoots !== null && job.pipeBoots !== undefined) lines.push(`${job.pipeBoots} pipe boot/penetration${job.pipeBoots === 1 ? "" : "s"} (priced)`);
  ...
  // Only worth a bullet here when it has been explicitly EXCLUDED - that is
  // the one state that deviates from the default and needs flagging.
  if (job.dumpsterRequested === false) lines.push(`no dumpster needed - customer/estimator supplying their own (disposal fee removed from base price)`);
  // Items below are captured from the estimator's text but are NOT
  // automatically priced anywhere in the system - there is no configured
  // company rate for them (complexity/access have no pricing_profiles
  // column at all; permit/ventilation/flashing vary too much by
  // municipality/job to safely auto-price). Labeled explicitly as
  ...
  if (job.permitRequired === true) lines.push(`permit noted as required (NOT priced - permit fees vary by municipality; call it out to the customer as a separate cost)`);
  if (job.ventilation) lines.push(`ventilation note: ${job.ventilation} (NOT priced - flag for manual review/quote)`);
  if (job.flashingScope) lines.push(`flashing note: ${job.flashingScope} (NOT priced beyond chimney flashing already counted above - flag for manual review if this is additional scope)`);
  return lines;
}
...
// Both paths use the same key names where the concept overlaps, so one
// formatter covers both.
